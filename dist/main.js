...
function run() {
    try {
        child_process_1.default.execSync(`echo "${process.env.INPUT_SSH_PRIVATE_KEY}" > ~/.ssh/id_rsa`);
        child_process_1.default.execSync(`echo "${process.env.INPUT_SSH_PUBLIC_KEY}" > ~/.ssh/id_rsa.pub`);
